...
import main.java.com.shop.dao.UserMapper;
...
public class UserService {
   ...
   //那保存对象进数据库的时候，就没必要放到缓存了
	...
	/**
	 * 测试接口验证
	 * @param map 参数信息，包含签名信息
	 * @return
	 */
	public boolean validate (Map<String,String> map) {
		String sign =map.get("sign");//获取到签名信息，后面需要进行验证
		map.remove("sign");//先移除签名信息，不然后面进行排序签名验证的时候回出错
		String params = SPaySignUtils.createLinkString(map);//将参数安装ASCILL码进行排序
		boolean bool= false;
		try {
			 bool= RSACoder.verify(params.getBytes(),RSACoder.PUBLIC_KEY,sign);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return bool;
